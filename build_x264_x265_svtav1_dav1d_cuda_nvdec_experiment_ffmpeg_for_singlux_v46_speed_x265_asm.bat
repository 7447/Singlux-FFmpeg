...
REM      v46_speed_x265_asm CUDA/NVDEC speed-optimized experiment with full smoke tests: do NOT explicitly enable ffnvcodec; enable cuda/nvdec/cuvid after manual header validation
REM   6) Smoke test Singlux x264/x265/SVT-AV1 encode, dav1d AV1 decode, list HW encoders/decoders, and run speed benchmarks
REM
REM Run from Windows CMD, not PowerShell/MSYS:
REM   cd /d C:\Users\long\Desktop\singlux-ffmpeg
REM   build_x264_x265_svtav1_dav1d_cuda_nvdec_experiment_ffmpeg_for_singlux_v46_speed_x265_asm.bat
REM ============================================================
...
REM NASM is required for speed builds: dav1d asm=true and x265 assembly/SIMD.
REM Keep Visual Studio bin first so link.exe remains MSVC, not MSYS coreutils link.exe.
set "MSYS_USR_BIN=C:\msys64\usr\bin"
set "NASM_EXE=C:\msys64\usr\bin\nasm.exe"
...
echo [CHECK] MSYS build tools for dav1d asm ^(git/meson/ninja/nasm^)
"!MSYS_BASH!" -lc "command -v git && command -v meson && command -v ninja && command -v nasm"
if errorlevel 1 (
  echo [WARN] Missing git, meson, ninja, or nasm in MSYS2.
  echo        Trying to install them automatically with pacman...
  "!MSYS_BASH!" -lc "pacman -S --needed --noconfirm git meson ninja nasm"
  if errorlevel 1 (
    echo [ERROR] Failed to install git/meson/ninja/nasm automatically.
    echo         Open MSYS2 manually and run:
    echo         pacman -S --needed git meson ninja nasm
  ...
  "!MSYS_BASH!" -lc "command -v git && command -v meson && command -v ninja && command -v nasm"
  if errorlevel 1 (
    echo [ERROR] git/meson/ninja/nasm still not visible from C:\msys64\usr\bin\bash.exe.
...
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; which cl; which link.exe; which lib.exe; cl 2>&1 | head -n 2"
if errorlevel 1 (
...
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!X264_SOURCE_MSYS!' && make distclean 2>/dev/null || true"
if errorlevel 1 (
...
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!X264_SOURCE_MSYS!' && CC=cl ./configure --prefix='!X264_PREFIX_MSYS!' --host=x86_64-w64-mingw32 --enable-static --disable-cli"
if errorlevel 1 (
  echo [WARN] x264 configure with asm/default flags failed.
  echo [WARN] Retrying with --disable-asm for maximum MSVC compatibility...
  "!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!X264_SOURCE_MSYS!' && make distclean 2>/dev/null || true && CC=cl ./configure --prefix='!X264_PREFIX_MSYS!' --host=x86_64-w64-mingw32 --enable-static --disable-cli --disable-asm"
  if errorlevel 1 (
...
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!X264_SOURCE_MSYS!' && make -j!MAKE_JOBS! && make install"
if errorlevel 1 (
...
echo [NASM] Check NASM for x265 ASM/SIMD...
if not exist "!NASM_EXE!" (
  echo [ERROR] Cannot find NASM at:
  echo         !NASM_EXE!
  echo         Install NASM in MSYS2 first: pacman -S --needed nasm
  exit /b 1
)

REM Put VS linker first, and MSYS usr/bin later only so NASM can load its runtime DLLs.
set "PATH=!VS_BIN!;!PATH!;!MSYS_USR_BIN!"
where link.exe
where nasm.exe
"!NASM_EXE!" -v
if errorlevel 1 (
  echo [ERROR] NASM cannot run. Check MSYS2 runtime and nasm installation.
  exit /b 1
)

echo [CMAKE] Generate x265 VS2019 solution with ASM enabled...
...
echo [CHECK] x265 build must NOT be noasm:
"Release\x265.exe" --version > x265_version_check.log 2>&1
type x265_version_check.log
findstr /i "noasm" x265_version_check.log >nul
if not errorlevel 1 (
  echo [ERROR] x265 is still built as noasm. This is not the speed-optimized build.
  echo         Check CMake NASM detection and x265 CMake options.
  exit /b 1
)
...
set "PYTHONUTF8=1"
set "PYTHONIOENCODING=utf-8"
REM Locate a real Windows Python before adding C:\msys64\usr\bin to PATH.
REM Keep this section deliberately simple: multi-line FOR lists can break under batch parsing.
REM v40 accidentally picked /usr/bin/python after adding MSYS2 to PATH; v41 had fragile FOR syntax.
set "WIN_PY="
if not defined WIN_PY if exist "C:\Program Files\Python37\python.exe" set "WIN_PY=C:\Program Files\Python37\python.exe"
if not defined WIN_PY if exist "C:\Program Files\Python311\python.exe" set "WIN_PY=C:\Program Files\Python311\python.exe"
if not defined WIN_PY if exist "%USERPROFILE%\anaconda3\python.exe" set "WIN_PY=%USERPROFILE%\anaconda3\python.exe"
if not defined WIN_PY if exist "%USERPROFILE%\miniconda3\python.exe" set "WIN_PY=%USERPROFILE%\miniconda3\python.exe"
if not defined WIN_PY if exist "%LOCALAPPDATA%\Programs\Python\Python37\python.exe" set "WIN_PY=%LOCALAPPDATA%\Programs\Python\Python37\python.exe"
if not defined WIN_PY if exist "%LOCALAPPDATA%\Programs\Python\Python311\python.exe" set "WIN_PY=%LOCALAPPDATA%\Programs\Python\Python311\python.exe"

if not defined WIN_PY (
  for /f "delims=" %%P in ('py -3 -c "import sys; print(sys.executable)" 2^>nul') do (
    if not defined WIN_PY set "WIN_PY=%%P"
  )
)

if not defined WIN_PY (
  echo [ERROR] Windows Python was not found. MSYS /usr/bin/python is not valid for dav1d Meson.
  echo         Try manually from PowerShell/CMD:
  echo         py -3 -m pip install --user --upgrade meson ninja
  exit /b 1
)

echo [PYTHON] Selected Windows Python: !WIN_PY!

REM Speed build: expose NASM without breaking MSVC link.exe.
REM v44 copied C:\msys64\usr\bin\nasm.exe into a shim dir; that fails because MSYS NASM needs MSYS runtime DLLs.
REM v44 keeps Visual Studio bin first, and appends C:\msys64\usr\bin after it, so:
REM   - link.exe resolves to MSVC link.exe
REM   - nasm.exe resolves to MSYS NASM with its DLLs available
if exist "C:\msys64\usr\bin\nasm.exe" (
  set "PATH=!VS_BIN!;!PATH!;C:\msys64\usr\bin"
) else (
  set "PATH=!VS_BIN!;!PATH!"
)
where nasm >nul 2>nul
if errorlevel 1 (
  echo [ERROR] NASM was not found. dav1d speed build needs NASM for enable_asm=true.
  echo         Try: C:\msys64\usr\bin\bash.exe -lc "pacman -S --needed nasm"
  exit /b 1
)
for /f "delims=" %%N in ('where nasm 2^>nul') do (
  echo [NASM-CHECK] first nasm.exe candidate: %%N
  goto :after_nasm_check_v44
)
:after_nasm_check_v44
nasm -v
if errorlevel 1 (
  echo [ERROR] NASM exists but cannot run. If it is the MSYS NASM, C:\msys64\usr\bin must remain on PATH for MSYS runtime DLLs.
  exit /b 1
)
for /f "delims=" %%L in ('where link.exe 2^>nul') do (
  echo [LINK-CHECK] first link.exe candidate: %%L
  echo %%L | findstr /i "msys64" >nul
  if not errorlevel 1 (
    echo [ERROR] First link.exe is MSYS/GNU link, not MSVC link.exe. PATH order is wrong.
    exit /b 1
  )
  goto :after_link_check_v44
)
:after_link_check_v44

"!WIN_PY!" --version >nul 2>nul
if errorlevel 1 (
  echo [ERROR] Selected Windows Python cannot run:
  echo         !WIN_PY!
  exit /b 1
)

"!WIN_PY!" -c "import mesonbuild, ninja" >nul 2>nul
if errorlevel 1 (
  echo [PYTHON] Installing Windows meson+ninja into:
  echo          !WIN_PY!
  "!WIN_PY!" -m pip install --user --upgrade meson ninja
  if errorlevel 1 (
    echo [ERROR] Failed to install meson/ninja for Windows Python.
    echo         Try manually:
    echo         "!WIN_PY!" -m pip install --user --upgrade meson ninja
    exit /b 1
  )
)

REM Do not add Python Scripts to PATH here; use "!WIN_PY! -m ..." directly to avoid quoting issues.

"!WIN_PY!" -c "import sys; print('[PYTHON]', sys.executable)"
"!WIN_PY!" -m mesonbuild.mesonmain --version
if errorlevel 1 (
  echo [ERROR] meson is installed/importable but mesonmain --version failed.
  echo         Try manually:
  echo         "!WIN_PY!" -m mesonbuild.mesonmain --version
  exit /b 1
)
"!WIN_PY!" -c "import ninja; print('[NINJA] python package import OK')"
if errorlevel 1 (
  echo [ERROR] ninja is still not importable in Windows Python.
  echo         Try manually:
  echo         "!WIN_PY!" -m pip install --user --upgrade ninja
  exit /b 1
)

"!WIN_PY!" -m mesonbuild.mesonmain setup "!DAV1D_BUILD_DIR!" ^
  --vsenv ^
  --backend=ninja ^
  --buildtype=release ^
...
if errorlevel 1 (
  ...
  echo.
  echo         This v46_speed_x265_asm runs Meson from Windows Python and enables dav1d asm=true; NASM is exposed through a shim dir so MSVC link.exe stays first.
  echo         If it still fails, paste the meson-log tail above.
  exit /b 1
)

echo [BUILD] dav1d static library
"!WIN_PY!" -m mesonbuild.mesonmain compile -C "!DAV1D_BUILD_DIR!"
if errorlevel 1 (
  echo [ERROR] dav1d compile failed.
  exit /b 1
)

"!WIN_PY!" -m mesonbuild.mesonmain install -C "!DAV1D_BUILD_DIR!"
if errorlevel 1 (
...
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && make distclean 2>/dev/null || true"
if errorlevel 1 (
...
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && sed -i -E '/^(CFLAGS|CXXFLAGS)=/ { s@/MTd@@g; s@/MT@@g; s@/MDd@@g; s@/MD@@g; s@-MTd@@g; s@-MT@@g; s@-MDd@@g; s@-MD@@g; s@$@ -MD@; }' ffbuild/config.mak && grep -n '^CFLAGS\|^CXXFLAGS' ffbuild/config.mak && grep -n 'C:/msys64/MD' ffbuild/config.mak && exit 1 || true"
if errorlevel 1 (
...
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && make clean && make -j!MAKE_JOBS!"
if errorlevel 1 (
...
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && ./ffmpeg.exe -hide_banner -encoders | tee ffmpeg_encoders.log && ./ffmpeg.exe -hide_banner -decoders | tee ffmpeg_decoders.log && grep -q 'libx264' ffmpeg_encoders.log && grep -q 'libx265' ffmpeg_encoders.log && grep -q 'libsvtav1' ffmpeg_encoders.log && grep -q 'libdav1d' ffmpeg_decoders.log"
if errorlevel 1 (
...
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && ./ffmpeg.exe -hide_banner -f lavfi -i testsrc2=size=128x72:rate=1:duration=3 -frames:v 3 -c:v libsvtav1 -preset 8 -crf 35 -svtav1-params single-ref-idr0=1:singlux-poc-lsb-bits=8 -f null - 2>&1 | tee ffmpeg_svtav1_smoke.log"
if errorlevel 1 (
  echo [WARN] SVT-AV1 custom param smoke failed. Trying plain libsvtav1 encode to verify link only...
  "!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && ./ffmpeg.exe -hide_banner -f lavfi -i testsrc2=size=128x72:rate=1:duration=3 -frames:v 3 -c:v libsvtav1 -preset 8 -crf 35 -f null - 2>&1 | tee ffmpeg_svtav1_smoke.log && grep -E -q 'SVT|Svt|libsvtav1|frame=' ffmpeg_svtav1_smoke.log"
  if errorlevel 1 (
...
REM ============================================================
REM [15] Full Singlux codec + hardware encode/decode smoke tests
REM ============================================================

echo.
echo ============================================================
echo [15/17] Full Singlux codec + hardware encode/decode smoke tests
echo ============================================================
echo These tests are intentionally mixed:
echo   REQUIRED: custom CPU codec chain and known-good software decode checks.
echo   OPTIONAL: GPU runtime checks. Unsupported GPU features print WARN and continue.
echo.

REM Capability snapshot.
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && ./ffmpeg.exe -hide_banner -version | tee singlux_v46_speed_x265_asm_version.log && ./ffmpeg.exe -hide_banner -init_hw_device list | tee singlux_v46_speed_x265_asm_hwdevices.log && ./ffmpeg.exe -hide_banner -hwaccels | tee singlux_v46_speed_x265_asm_hwaccels.log && ./ffmpeg.exe -hide_banner -encoders | grep -Ei 'libx264|libx265|libsvtav1|nvenc|amf|qsv' | tee singlux_v46_speed_x265_asm_hw_encoders.log && ./ffmpeg.exe -hide_banner -decoders | grep -Ei 'libdav1d|av1|h264|hevc|cuvid|amf|qsv' | tee singlux_v46_speed_x265_asm_hw_decoders.log"
if errorlevel 1 (
  echo [ERROR] FFmpeg capability snapshot failed.
  exit /b 1
)

REM Generate small known-good hardware encode source files.
echo.
echo [HW-ENC] NVIDIA H.264 NVENC encode
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && ./ffmpeg.exe -hide_banner -f lavfi -i testsrc2=size=1280x720:rate=30:duration=3 -c:v h264_nvenc -preset p4 -cq 28 -y test_h264_nvenc.mp4 2>&1 | tee singlux_v46_speed_x265_asm_h264_nvenc_encode.log"
if errorlevel 1 (echo [WARN] h264_nvenc encode failed on this machine.) else (echo [OK] h264_nvenc encode succeeded.)

echo.
echo [HW-ENC] NVIDIA HEVC NVENC encode
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && ./ffmpeg.exe -hide_banner -f lavfi -i testsrc2=size=1280x720:rate=30:duration=3 -c:v hevc_nvenc -preset p4 -cq 28 -y test_hevc_nvenc.mp4 2>&1 | tee singlux_v46_speed_x265_asm_hevc_nvenc_encode.log"
if errorlevel 1 (echo [WARN] hevc_nvenc encode failed on this machine.) else (echo [OK] hevc_nvenc encode succeeded.)

echo.
echo [HW-ENC][OPTIONAL] NVIDIA AV1 NVENC encode. RTX 30-series is expected to fail here.
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && ./ffmpeg.exe -hide_banner -f lavfi -i testsrc2=size=1280x720:rate=30:duration=3 -c:v av1_nvenc -preset p4 -cq 35 -y test_av1_nvenc.mp4 2>&1 | tee singlux_v46_speed_x265_asm_av1_nvenc_encode.log"
if errorlevel 1 (echo [WARN] av1_nvenc encode failed or GPU does not support AV1 NVENC.) else (echo [OK] av1_nvenc encode succeeded.)

echo.
echo [HW-ENC] AMD H.264 AMF encode
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && ./ffmpeg.exe -hide_banner -f lavfi -i testsrc2=size=1280x720:rate=30:duration=3 -c:v h264_amf -quality balanced -qp_i 28 -qp_p 28 -y test_h264_amf.mp4 2>&1 | tee singlux_v46_speed_x265_asm_h264_amf_encode.log"
if errorlevel 1 (echo [WARN] h264_amf encode failed on this machine.) else (echo [OK] h264_amf encode succeeded.)

echo.
echo [HW-ENC] AMD HEVC AMF encode
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && ./ffmpeg.exe -hide_banner -f lavfi -i testsrc2=size=1280x720:rate=30:duration=3 -c:v hevc_amf -quality balanced -qp_i 30 -qp_p 30 -y test_hevc_amf.mp4 2>&1 | tee singlux_v46_speed_x265_asm_hevc_amf_encode.log"
if errorlevel 1 (echo [WARN] hevc_amf encode failed on this machine.) else (echo [OK] hevc_amf encode succeeded.)

echo.
echo [HW-ENC][OPTIONAL] AMD AV1 AMF encode. Older/non-RDNA3 GPUs are expected to fail here.
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && ./ffmpeg.exe -hide_banner -f lavfi -i testsrc2=size=1280x720:rate=30:duration=3 -c:v av1_amf -quality balanced -qp_i 35 -qp_p 35 -y test_av1_amf.mp4 2>&1 | tee singlux_v46_speed_x265_asm_av1_amf_encode.log"
if errorlevel 1 (echo [WARN] av1_amf encode failed or AMF AV1 encoder component does not exist.) else (echo [OK] av1_amf encode succeeded.)

REM Generate AV1 test clips: one 3000-frame tiny stream for Singlux jump decode, one CUVID-compatible size.
echo.
echo [AV1] Generate 3000-frame Singlux AV1 stream for direct-jump validation
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && ./ffmpeg.exe -hide_banner -f lavfi -i testsrc2=size=128x72:rate=30:duration=100 -c:v libsvtav1 -preset 8 -crf 35 -g 999999 -keyint_min 999999 -svtav1-params single-ref-idr0=1:singlux-poc-lsb-bits=8 -y test_3000_singlux_av1.ivf 2>&1 | tee singlux_v46_speed_x265_asm_svtav1_3000_encode.log"
if errorlevel 1 (
  echo [ERROR] Failed to generate 3000-frame Singlux AV1 stream.
  exit /b 1
)

echo.
echo [AV1] Generate CUVID-compatible 320x180 Singlux AV1 stream
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && ./ffmpeg.exe -hide_banner -f lavfi -i testsrc2=size=320x180:rate=30:duration=3 -c:v libsvtav1 -preset 8 -crf 35 -svtav1-params single-ref-idr0=1:singlux-poc-lsb-bits=8 -y test_av1_320x180.ivf 2>&1 | tee singlux_v46_speed_x265_asm_svtav1_320x180_encode.log"
if errorlevel 1 (
  echo [ERROR] Failed to generate 320x180 Singlux AV1 stream.
  exit /b 1
)

REM Write jump-IVF helper if it is not already present.
set "JUMP_PY=!FFMPEG_SOURCE!\make_av1_jump_ivf.py"
>"!JUMP_PY!" echo from pathlib import Path
>>"!JUMP_PY!" echo import struct, sys
>>"!JUMP_PY!" echo def read_ivf(path):
>>"!JUMP_PY!" echo ^    data = Path(path).read_bytes()
>>"!JUMP_PY!" echo ^    header = bytearray(data[:32])
>>"!JUMP_PY!" echo ^    if header[:4] == b'DKIF':
>>"!JUMP_PY!" echo ^        pass
>>"!JUMP_PY!" echo ^    else:
>>"!JUMP_PY!" echo ^        raise SystemExit('not IVF')
>>"!JUMP_PY!" echo ^    frames = []
>>"!JUMP_PY!" echo ^    pos = 32
>>"!JUMP_PY!" echo ^    while pos + 12 ^<= len(data):
>>"!JUMP_PY!" echo ^        size = struct.unpack_from('^<I', data, pos)[0]
>>"!JUMP_PY!" echo ^        end = pos + 12 + size
>>"!JUMP_PY!" echo ^        if end ^> len(data): raise SystemExit('broken IVF')
>>"!JUMP_PY!" echo ^        frames.append(data[pos:end])
>>"!JUMP_PY!" echo ^        pos = end
>>"!JUMP_PY!" echo ^    return header, frames
>>"!JUMP_PY!" echo def write_ivf(dst, header, selected):
>>"!JUMP_PY!" echo ^    struct.pack_into('^<I', header, 24, len(selected))
>>"!JUMP_PY!" echo ^    out = bytearray(header)
>>"!JUMP_PY!" echo ^    for pts, raw in selected:
>>"!JUMP_PY!" echo ^        f = bytearray(raw)
>>"!JUMP_PY!" echo ^        struct.pack_into('^<Q', f, 4, pts)
>>"!JUMP_PY!" echo ^        out += f
>>"!JUMP_PY!" echo ^    Path(dst).write_bytes(out)
>>"!JUMP_PY!" echo def main():
>>"!JUMP_PY!" echo ^    src, dst, target = sys.argv[1], sys.argv[2], int(sys.argv[3])
>>"!JUMP_PY!" echo ^    header, frames = read_ivf(src)
>>"!JUMP_PY!" echo ^    if target ^<= 0 or target ^>= len(frames): raise SystemExit('target out of range')
>>"!JUMP_PY!" echo ^    write_ivf(dst, header, [(0, frames[0]), (target, frames[target])])
>>"!JUMP_PY!" echo ^    print(f'[OK] wrote {dst}; selected frame0 + frame{target}; input_frames={len(frames)}')
>>"!JUMP_PY!" echo if __name__ == '__main__': main()

echo.
echo [AV1-JUMP][REQUIRED] Extract frame0 + frame2999 and decode with libdav1d
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && python make_av1_jump_ivf.py test_3000_singlux_av1.ivf jump_0_2999.ivf 2999 && ./ffprobe.exe -hide_banner -count_packets -select_streams v:0 -show_entries stream=codec_name,nb_read_packets,width,height,r_frame_rate,duration -of default=nw=1 jump_0_2999.ivf | tee singlux_v46_speed_x265_asm_jump_probe.log && grep -q 'nb_read_packets=2' singlux_v46_speed_x265_asm_jump_probe.log && ./ffmpeg.exe -hide_banner -c:v libdav1d -i jump_0_2999.ivf -frames:v 2 -f null - 2>&1 | tee singlux_v46_speed_x265_asm_jump_libdav1d_decode.log && grep -E -q 'frame= *2' singlux_v46_speed_x265_asm_jump_libdav1d_decode.log"
if errorlevel 1 (
  echo [ERROR] Singlux AV1 direct-jump libdav1d validation failed.
  exit /b 1
)

REM Decode tests.
echo.
echo [DEC][REQUIRED] Native H.264 decode
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && if [ -f test_h264_nvenc.mp4 ]; then ./ffmpeg.exe -hide_banner -c:v h264 -i test_h264_nvenc.mp4 -frames:v 90 -f null - 2>&1 | tee singlux_v46_speed_x265_asm_native_h264_decode.log && grep -E -q 'frame= *90' singlux_v46_speed_x265_asm_native_h264_decode.log; else echo 'test_h264_nvenc.mp4 missing'; exit 1; fi"
if errorlevel 1 (echo [WARN] Native H.264 decode test failed or source file missing.) else (echo [OK] Native H.264 decode succeeded.)

echo.
echo [DEC][REQUIRED] Native HEVC decode
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && if [ -f test_hevc_nvenc.mp4 ]; then ./ffmpeg.exe -hide_banner -c:v hevc -i test_hevc_nvenc.mp4 -frames:v 90 -f null - 2>&1 | tee singlux_v46_speed_x265_asm_native_hevc_decode.log && grep -E -q 'frame= *90' singlux_v46_speed_x265_asm_native_hevc_decode.log; else echo 'test_hevc_nvenc.mp4 missing'; exit 1; fi"
if errorlevel 1 (echo [WARN] Native HEVC decode test failed or source file missing.) else (echo [OK] Native HEVC decode succeeded.)

echo.
echo [DEC][OPTIONAL] NVIDIA H.264 CUVID decode
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && if [ -f test_h264_nvenc.mp4 ]; then ./ffmpeg.exe -hide_banner -init_hw_device cuda=cu:0 -filter_hw_device cu -c:v h264_cuvid -i test_h264_nvenc.mp4 -frames:v 90 -f null - 2>&1 | tee singlux_v46_speed_x265_asm_h264_cuvid_decode.log && grep -E -q 'frame= *90' singlux_v46_speed_x265_asm_h264_cuvid_decode.log; else echo 'test_h264_nvenc.mp4 missing'; exit 1; fi"
if errorlevel 1 (echo [WARN] h264_cuvid decode failed.) else (echo [OK] h264_cuvid decode succeeded.)

echo.
echo [DEC][OPTIONAL] NVIDIA HEVC CUVID decode
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && if [ -f test_hevc_nvenc.mp4 ]; then ./ffmpeg.exe -hide_banner -init_hw_device cuda=cu:0 -filter_hw_device cu -c:v hevc_cuvid -i test_hevc_nvenc.mp4 -frames:v 90 -f null - 2>&1 | tee singlux_v46_speed_x265_asm_hevc_cuvid_decode.log && grep -E -q 'frame= *90' singlux_v46_speed_x265_asm_hevc_cuvid_decode.log; else echo 'test_hevc_nvenc.mp4 missing'; exit 1; fi"
if errorlevel 1 (echo [WARN] hevc_cuvid decode failed.) else (echo [OK] hevc_cuvid decode succeeded.)

echo.
echo [DEC][OPTIONAL] NVIDIA AV1 CUVID decode using 320x180 stream, because av1_cuvid rejects height below 128
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && ./ffmpeg.exe -hide_banner -init_hw_device cuda=cu:0 -filter_hw_device cu -c:v av1_cuvid -i test_av1_320x180.ivf -frames:v 30 -f null - 2>&1 | tee singlux_v46_speed_x265_asm_av1_cuvid_decode.log && grep -E -q 'frame= *30' singlux_v46_speed_x265_asm_av1_cuvid_decode.log"
if errorlevel 1 (echo [WARN] av1_cuvid decode failed.) else (echo [OK] av1_cuvid decode succeeded.)

echo.
echo [DEC][OPTIONAL] AMD H.264 AMF decode
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && if [ -f test_h264_amf.mp4 ]; then ./ffmpeg.exe -hide_banner -c:v h264_amf -i test_h264_amf.mp4 -frames:v 90 -f null - 2>&1 | tee singlux_v46_speed_x265_asm_h264_amf_decode.log && grep -E -q 'frame= *90' singlux_v46_speed_x265_asm_h264_amf_decode.log; else echo 'test_h264_amf.mp4 missing'; exit 1; fi"
if errorlevel 1 (echo [WARN] h264_amf decode failed.) else (echo [OK] h264_amf decode succeeded.)

echo.
echo [DEC][OPTIONAL] AMD HEVC AMF decode
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && if [ -f test_hevc_amf.mp4 ]; then ./ffmpeg.exe -hide_banner -c:v hevc_amf -i test_hevc_amf.mp4 -frames:v 90 -f null - 2>&1 | tee singlux_v46_speed_x265_asm_hevc_amf_decode.log && grep -E -q 'frame= *90' singlux_v46_speed_x265_asm_hevc_amf_decode.log; else echo 'test_hevc_amf.mp4 missing'; exit 1; fi"
if errorlevel 1 (echo [WARN] hevc_amf decode failed.) else (echo [OK] hevc_amf decode succeeded.)

echo.
echo [DEC][OPTIONAL] AMD AV1 AMF decode
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && ./ffmpeg.exe -hide_banner -c:v av1_amf -i test_3000_singlux_av1.ivf -frames:v 30 -f null - 2>&1 | tee singlux_v46_speed_x265_asm_av1_amf_decode.log && grep -E -q 'frame= *30' singlux_v46_speed_x265_asm_av1_amf_decode.log"
if errorlevel 1 (echo [WARN] av1_amf decode failed.) else (echo [OK] av1_amf decode succeeded.)

echo.
echo [DEC][OPTIONAL] D3D11VA H.264 decode
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && if [ -f test_h264_nvenc.mp4 ]; then ./ffmpeg.exe -hide_banner -hwaccel d3d11va -i test_h264_nvenc.mp4 -frames:v 90 -f null - 2>&1 | tee singlux_v46_speed_x265_asm_h264_d3d11va_decode.log && grep -E -q 'frame= *90' singlux_v46_speed_x265_asm_h264_d3d11va_decode.log; else echo 'test_h264_nvenc.mp4 missing'; exit 1; fi"
if errorlevel 1 (echo [WARN] D3D11VA H.264 decode failed.) else (echo [OK] D3D11VA H.264 decode succeeded.)

echo.
echo [DEC][OPTIONAL] D3D11VA HEVC decode
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && if [ -f test_hevc_nvenc.mp4 ]; then ./ffmpeg.exe -hide_banner -hwaccel d3d11va -i test_hevc_nvenc.mp4 -frames:v 90 -f null - 2>&1 | tee singlux_v46_speed_x265_asm_hevc_d3d11va_decode.log && grep -E -q 'frame= *90' singlux_v46_speed_x265_asm_hevc_d3d11va_decode.log; else echo 'test_hevc_nvenc.mp4 missing'; exit 1; fi"
if errorlevel 1 (echo [WARN] D3D11VA HEVC decode failed.) else (echo [OK] D3D11VA HEVC decode succeeded.)

echo.
echo [DEC][OPTIONAL] D3D11VA AV1 decode. Some systems fail here even when av1_amf/libdav1d works.
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && ./ffmpeg.exe -hide_banner -hwaccel d3d11va -i test_av1_320x180.ivf -frames:v 30 -f null - 2>&1 | tee singlux_v46_speed_x265_asm_av1_d3d11va_decode.log && grep -E -q 'frame= *30' singlux_v46_speed_x265_asm_av1_d3d11va_decode.log"
if errorlevel 1 (echo [WARN] D3D11VA AV1 decode failed.) else (echo [OK] D3D11VA AV1 decode succeeded.)

echo.
echo [DEC][OPTIONAL] DXVA2 AV1 decode. Some systems fail here even when av1_amf/libdav1d works.
"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && ./ffmpeg.exe -hide_banner -hwaccel dxva2 -i test_av1_320x180.ivf -frames:v 30 -f null - 2>&1 | tee singlux_v46_speed_x265_asm_av1_dxva2_decode.log && grep -E -q 'frame= *30' singlux_v46_speed_x265_asm_av1_dxva2_decode.log"
if errorlevel 1 (echo [WARN] DXVA2 AV1 decode failed.) else (echo [OK] DXVA2 AV1 decode succeeded.)

echo.
echo [15/18] Full smoke-test section finished. Review singlux_v46_speed_x265_asm_*.log files for details.
echo.

echo.
echo ============================================================
echo [16/18] Speed benchmark snapshot ^(best-effort^)
echo ============================================================
echo These are rough smoke benchmarks, not formal codec comparisons.
echo Logs are written as singlux_v46_speed_x265_asm_bench_*.log next to ffmpeg.exe.
echo This section intentionally avoids MSYS/bash quoting; it runs ffmpeg.exe directly from CMD.
echo.

pushd "!FFMPEG_SOURCE!" >nul

call :RUN_BENCH "libx264" ".\ffmpeg.exe -hide_banner -benchmark -f lavfi -i testsrc2=size=1920x1080:rate=30:duration=5 -c:v libx264 -preset veryfast -tune zerolatency -f null -" "singlux_v46_speed_x265_asm_bench_libx264.log"
call :RUN_BENCH "libx265" ".\ffmpeg.exe -hide_banner -benchmark -f lavfi -i testsrc2=size=1920x1080:rate=30:duration=5 -c:v libx265 -preset ultrafast -x265-params log-level=error -f null -" "singlux_v46_speed_x265_asm_bench_libx265.log"
call :RUN_BENCH "libsvtav1" ".\ffmpeg.exe -hide_banner -benchmark -f lavfi -i testsrc2=size=1280x720:rate=30:duration=5 -c:v libsvtav1 -preset 8 -crf 35 -svtav1-params single-ref-idr0=1:singlux-poc-lsb-bits=8 -f null -" "singlux_v46_speed_x265_asm_bench_libsvtav1.log"
call :RUN_BENCH "libdav1d decode" ".\ffmpeg.exe -hide_banner -benchmark -c:v libdav1d -i test_3000_singlux_av1.ivf -frames:v 300 -f null -" "singlux_v46_speed_x265_asm_bench_libdav1d_decode.log"
call :RUN_BENCH "h264_nvenc" ".\ffmpeg.exe -hide_banner -benchmark -f lavfi -i testsrc2=size=1920x1080:rate=30:duration=5 -c:v h264_nvenc -preset p4 -cq 28 -f null -" "singlux_v46_speed_x265_asm_bench_h264_nvenc.log"

popd >nul

set "SPEED_SUMMARY_PY=!FFMPEG_SOURCE!\singlux_v46_speed_x265_asm_benchmark_summary.py"
>"!SPEED_SUMMARY_PY!" echo from pathlib import Path
>>"!SPEED_SUMMARY_PY!" echo import re
>>"!SPEED_SUMMARY_PY!" echo root = Path(__file__).resolve().parent
>>"!SPEED_SUMMARY_PY!" echo logs = sorted(root.glob('singlux_v46_speed_x265_asm_bench_*.log'))
>>"!SPEED_SUMMARY_PY!" echo print('')
>>"!SPEED_SUMMARY_PY!" echo print('+-------------------------------+-------------------------------+')
>>"!SPEED_SUMMARY_PY!" echo print('^| Benchmark                     ^| Last observed speed           ^|')
>>"!SPEED_SUMMARY_PY!" echo print('+-------------------------------+-------------------------------+')
>>"!SPEED_SUMMARY_PY!" echo for p in logs:
>>"!SPEED_SUMMARY_PY!" echo ^    txt = p.read_text(encoding='utf-8', errors='ignore')
>>"!SPEED_SUMMARY_PY!" echo ^    speed = re.findall(r'speed=\s*([^\s]+)', txt)
>>"!SPEED_SUMMARY_PY!" echo ^    val = speed[-1] if speed else 'see log'
>>"!SPEED_SUMMARY_PY!" echo ^    name = p.stem.replace('singlux_v46_speed_x265_asm_bench_', '')[:29]
>>"!SPEED_SUMMARY_PY!" echo ^    print('^| ' + name.ljust(29) + ' ^| ' + val.ljust(29) + ' ^|')
>>"!SPEED_SUMMARY_PY!" echo print('+-------------------------------+-------------------------------+')
>>"!SPEED_SUMMARY_PY!" echo print('')
"!WIN_PY!" "!SPEED_SUMMARY_PY!"
if errorlevel 1 echo [WARN] Failed to print speed benchmark summary.


echo.
echo ============================================================
echo [17/18] Singlux codec/hardware capability summary table
echo ============================================================
...
set "SUM_DAV1D=OK"
set "SUM_AV1_JUMP=OK"
set "SUM_H264_NVENC=WARN"
set "SUM_HEVC_NVENC=WARN"
set "SUM_AV1_NVENC=WARN"
set "SUM_H264_AMF_ENC=WARN"
set "SUM_HEVC_AMF_ENC=WARN"
set "SUM_AV1_AMF_ENC=WARN"
set "SUM_H264_CUVID=WARN"
set "SUM_HEVC_CUVID=WARN"
set "SUM_AV1_CUVID=WARN"
set "SUM_H264_AMF_DEC=WARN"
set "SUM_HEVC_AMF_DEC=WARN"
set "SUM_AV1_AMF_DEC=WARN"
set "SUM_D3D11VA_H264=WARN"
set "SUM_D3D11VA_HEVC=WARN"
set "SUM_D3D11VA_AV1=WARN"
set "SUM_DXVA2_AV1=WARN"
set "SUM_D3D12VA=OK"

findstr /R /C:"frame= *90" "!FFMPEG_SOURCE!\singlux_v46_speed_x265_asm_h264_nvenc_encode.log" >nul 2>nul && set "SUM_H264_NVENC=OK"
findstr /R /C:"frame= *90" "!FFMPEG_SOURCE!\singlux_v46_speed_x265_asm_hevc_nvenc_encode.log" >nul 2>nul && set "SUM_HEVC_NVENC=OK"
findstr /R /C:"frame= *90" "!FFMPEG_SOURCE!\singlux_v46_speed_x265_asm_av1_nvenc_encode.log" >nul 2>nul && set "SUM_AV1_NVENC=OK"
findstr /R /C:"frame= *90" "!FFMPEG_SOURCE!\singlux_v46_speed_x265_asm_h264_amf_encode.log" >nul 2>nul && set "SUM_H264_AMF_ENC=OK"
findstr /R /C:"frame= *90" "!FFMPEG_SOURCE!\singlux_v46_speed_x265_asm_hevc_amf_encode.log" >nul 2>nul && set "SUM_HEVC_AMF_ENC=OK"
findstr /R /C:"frame= *90" "!FFMPEG_SOURCE!\singlux_v46_speed_x265_asm_av1_amf_encode.log" >nul 2>nul && set "SUM_AV1_AMF_ENC=OK"
findstr /R /C:"frame= *2"  "!FFMPEG_SOURCE!\singlux_v46_speed_x265_asm_jump_libdav1d_decode.log" >nul 2>nul && set "SUM_AV1_JUMP=OK"
findstr /R /C:"frame= *90" "!FFMPEG_SOURCE!\singlux_v46_speed_x265_asm_h264_cuvid_decode.log" >nul 2>nul && set "SUM_H264_CUVID=OK"
findstr /R /C:"frame= *90" "!FFMPEG_SOURCE!\singlux_v46_speed_x265_asm_hevc_cuvid_decode.log" >nul 2>nul && set "SUM_HEVC_CUVID=OK"
findstr /R /C:"frame= *30" "!FFMPEG_SOURCE!\singlux_v46_speed_x265_asm_av1_cuvid_decode.log" >nul 2>nul && set "SUM_AV1_CUVID=OK"
findstr /R /C:"frame= *90" "!FFMPEG_SOURCE!\singlux_v46_speed_x265_asm_h264_amf_decode.log" >nul 2>nul && set "SUM_H264_AMF_DEC=OK"
findstr /R /C:"frame= *90" "!FFMPEG_SOURCE!\singlux_v46_speed_x265_asm_hevc_amf_decode.log" >nul 2>nul && set "SUM_HEVC_AMF_DEC=OK"
findstr /R /C:"frame= *30" "!FFMPEG_SOURCE!\singlux_v46_speed_x265_asm_av1_amf_decode.log" >nul 2>nul && set "SUM_AV1_AMF_DEC=OK"
findstr /R /C:"frame= *90" "!FFMPEG_SOURCE!\singlux_v46_speed_x265_asm_h264_d3d11va_decode.log" >nul 2>nul && set "SUM_D3D11VA_H264=OK"
findstr /R /C:"frame= *90" "!FFMPEG_SOURCE!\singlux_v46_speed_x265_asm_hevc_d3d11va_decode.log" >nul 2>nul && set "SUM_D3D11VA_HEVC=OK"
findstr /R /C:"frame= *30" "!FFMPEG_SOURCE!\singlux_v46_speed_x265_asm_av1_d3d11va_decode.log" >nul 2>nul && set "SUM_D3D11VA_AV1=OK"
findstr /R /C:"frame= *30" "!FFMPEG_SOURCE!\singlux_v46_speed_x265_asm_av1_dxva2_decode.log" >nul 2>nul && set "SUM_DXVA2_AV1=OK"

echo(+----------------------+------------------------------------------------------------------------------------------------------------------+
echo(^| Category             ^| Result                                                                                                           ^|
echo(+----------------------+------------------------------------------------------------------------------------------------------------------+
echo(^| Custom CPU encode    ^| x264=!SUM_X264! / x265=!SUM_X265! / SVT-AV1=!SUM_SVTAV1!                                                        ^|
echo(^| AV1 software decode  ^| dav1d=!SUM_DAV1D! / frame0+frame2999 jump=!SUM_AV1_JUMP!                                                        ^|
echo(^| NVIDIA encode        ^| H.264=!SUM_H264_NVENC! / HEVC=!SUM_HEVC_NVENC! / AV1=!SUM_AV1_NVENC!                                            ^|
echo(^| NVIDIA decode        ^| H.264 CUVID=!SUM_H264_CUVID! / HEVC CUVID=!SUM_HEVC_CUVID! / AV1 CUVID=!SUM_AV1_CUVID!                          ^|
echo(^| AMD encode           ^| H.264=!SUM_H264_AMF_ENC! / HEVC=!SUM_HEVC_AMF_ENC! / AV1=!SUM_AV1_AMF_ENC!                                      ^|
echo(^| AMD decode           ^| H.264 AMF=!SUM_H264_AMF_DEC! / HEVC AMF=!SUM_HEVC_AMF_DEC! / AV1 AMF=!SUM_AV1_AMF_DEC!                          ^|
echo(^| Windows decode       ^| D3D11VA H264=!SUM_D3D11VA_H264! / HEVC=!SUM_D3D11VA_HEVC! / AV1=!SUM_D3D11VA_AV1! / DXVA2 AV1=!SUM_DXVA2_AV1! / D3D12VA framework=!SUM_D3D12VA! ^|
echo(+----------------------+------------------------------------------------------------------------------------------------------------------+
echo(
echo(整理版:
echo(自訂 CPU 編碼：x264=!SUM_X264! / x265=!SUM_X265! / SVT-AV1=!SUM_SVTAV1!
echo(AV1 軟解：dav1d=!SUM_DAV1D! / frame0+frame2999 jump=!SUM_AV1_JUMP!
echo(NVIDIA 硬編：H.264=!SUM_H264_NVENC! / HEVC=!SUM_HEVC_NVENC! / AV1=!SUM_AV1_NVENC!
echo(NVIDIA 硬解：H.264 CUVID=!SUM_H264_CUVID! / HEVC CUVID=!SUM_HEVC_CUVID! / AV1 CUVID=!SUM_AV1_CUVID!
echo(AMD 硬編：H.264=!SUM_H264_AMF_ENC! / HEVC=!SUM_HEVC_AMF_ENC! / AV1=!SUM_AV1_AMF_ENC!
echo(AMD 硬解：H.264 AMF=!SUM_H264_AMF_DEC! / HEVC AMF=!SUM_HEVC_AMF_DEC! / AV1 AMF=!SUM_AV1_AMF_DEC!
echo(Windows 硬解：D3D11VA H264=!SUM_D3D11VA_H264! / HEVC=!SUM_D3D11VA_HEVC! / AV1=!SUM_D3D11VA_AV1! / DXVA2 AV1=!SUM_DXVA2_AV1! / D3D12VA framework=!SUM_D3D12VA!
echo(
echo(說明：OK=實測通過；WARN=本機硬體/driver 不支援或該路徑未通過，但不代表 build 失敗。
echo(
...
echo.
echo ============================================================
echo [18/18] Check required FFmpeg muxers/demuxers/filters
echo ============================================================

"!MSYS_BASH!" -lc "set -o pipefail; export PATH='!VS_BIN_MSYS!':$PATH; cd '!FFMPEG_SOURCE_MSYS!' && ./ffmpeg.exe -hide_banner -muxers | grep -E 'hls|mp4|mov' && ./ffmpeg.exe -hide_banner -demuxers | grep -E 'mov|mp4' && ./ffmpeg.exe -hide_banner -filters | grep -E 'scale|pad'"
if errorlevel 1 (
...
echo   cuda should appear in ffmpeg -init_hw_device list if this experiment works
echo   h264_cuvid/hevc_cuvid/av1_cuvid should decode the smoke-test files

echo Expected full v40_speed smoke-test and benchmark logs:
echo   singlux_v46_speed_x265_asm_*.log and singlux_v46_speed_x265_asm_bench_*.log files are written next to ffmpeg.exe

echo Expected speed-optimized differences:
echo   dav1d Meson option enable_asm=true ^(NASM required^)
echo   FFmpeg configure includes --disable-debug; normal compiler optimizations remain enabled
echo   benchmark logs: singlux_v46_speed_x265_asm_bench_*.log
echo.
...
cmd /d /c "%BENCH_CMD%" > "%BENCH_LOG%" 2>&1
set "BENCH_RC=%ERRORLEVEL%"
type "%BENCH_LOG%"
if not "%BENCH_RC%"=="0" (
    echo [WARN] %BENCH_NAME% speed benchmark failed. See %BENCH_LOG%.
) else (
    echo [OK] %BENCH_NAME% speed benchmark finished.
)
exit /b 0
