...
REM      v32: do not explicitly --enable-ffnvcodec; use headers + patched nvenc/cuvid deps to avoid MSVC pkg-config false negative
...
echo.
echo ============================================================
echo [7/15] Configure FFmpeg with custom x264 + custom x265 + Singlux SVT-AV1 + dav1d + NVENC/CUVID + AMF + optional QSV
echo ============================================================
...
echo [PATCH] FFmpeg configure: bypass fragile MSVC probes for libsvtav1/libdav1d/ffnvcodec and ffnvcodec deps
REM v32: Do NOT pass --enable-ffnvcodec explicitly. FFmpeg treats ffnvcodec as
...
set "SVTAV1_PATCH_PY=%TEMP%\singlux_patch_ffmpeg_hwaccel_probe.py"
...
>>"!SVTAV1_PATCH_PY!" echo backup = p.with_name('configure.singlux_before_hwaccel_probe_patch')
...
>>"!SVTAV1_PATCH_PY!" echo already_ffn_probe = any(('enabled ffnvcodec' in x and 'check_headers ffnvcodec/nvEncodeAPI.h' in x) for x in lines)
>>"!SVTAV1_PATCH_PY!" echo already_ffn_deps = any(('singlux: ffnvcodec dependency bypass' in x) for x in lines)
>>"!SVTAV1_PATCH_PY!" echo dep_names = ('nvenc_deps', 'cuvid_deps', 'nvdec_deps')
...
>>"!SVTAV1_PATCH_PY!" echo         changed += 1
>>"!SVTAV1_PATCH_PY!" echo     elif 'enabled ffnvcodec' in line and 'require_pkg_config' in line and 'ffnvcodec' in line:
>>"!SVTAV1_PATCH_PY!" echo         out.append('enabled ffnvcodec       ^&^& check_headers ffnvcodec/nvEncodeAPI.h')
>>"!SVTAV1_PATCH_PY!" echo         changed += 1
...
>>"!SVTAV1_PATCH_PY!" echo         out.append('# singlux: ffnvcodec dependency bypass after manual header validation')
...
>>"!SVTAV1_PATCH_PY!" echo     print('patched %%d libsvtav1/libdav1d/ffnvcodec/nvenc-cuvid configure line(s)' %% changed)
>>"!SVTAV1_PATCH_PY!" echo elif already_svt and already_dav and (already_ffn_probe or already_ffn_deps):
>>"!SVTAV1_PATCH_PY!" echo     print('configure already patched for Singlux svtav1/dav1d/ffnvcodec/nvenc-cuvid; continuing')
>>"!SVTAV1_PATCH_PY!" echo else:
>>"!SVTAV1_PATCH_PY!" echo     print('[WARN] did not find old libsvtav1/libdav1d/ffnvcodec/nvenc-cuvid lines; continuing without patch')
...
if errorlevel 1 (
  echo [ERROR] Failed to patch FFmpeg configure for Singlux hwaccel probes.
  echo         Patch script was:
  type "!SVTAV1_PATCH_PY!"
  exit /b 1
)

echo [CHECK] Manual ffnvcodec header check for NVENC/CUVID before bypassing configure dependency
...
echo [CHECK] FFmpeg configure selected libsvtav1, libdav1d, nvenc/cuvid/amf when available
